...
function getCloudClient() {
  ...
  const url = normalizeSupabaseProjectUrl(SUPABASE_PROJECT_URL);
  if (!url || !SUPABASE_PUBLISHABLE_KEY || !window.supabase?.createClient) return null;
  cloudClient = window.supabase.createClient(url, SUPABASE_PUBLISHABLE_KEY, {
    auth: {
...
function isCloudAdmin() {
  return cloudProfile?.role === 'admin';
}
...
function getCloudAccountSnapshot() {
  const user = getCloudUser();
  if (!user) return null;
  return {
...
function updateCloudUi() {
  ...
  if (status) {
    if (!SUPABASE_PROJECT_URL || !SUPABASE_PUBLISHABLE_KEY) {
      status.innerHTML = 'Supabase接続設定が未入力です。';
    } else if (!window.supabase?.createClient) {
      status.innerHTML = 'Supabase SDKを読み込めませんでした。ネットワークまたはCDNを確認してください。';
    } else if (!loggedIn) {
      status.innerHTML = 'クラウド未ログインです。メールアドレスとパスワードでログインしてください。';
    } else {
      const roleLabel = isCloudAdmin() ? '管理者' : 'プレイヤー';
      status.innerHTML = `ログイン中: <strong>${escapeHtml(getCloudDisplayName())}</strong><br><span>${escapeHtml(user.email || user.id)} / ${roleLabel}</span>`;
    }
  }

  const note = $('#cloudAccountNote');
  if (note) {
    if (cloudLastError) note.textContent = `クラウドエラー: ${cloudLastError}`;
...
function formatCloudOwner(row) {
  const profile = Array.isArray(row?.profiles) ? row.profiles[0] : row?.profiles;
  return profile?.display_name
    || row?.data?.profile?.playerName
    || row?.data?.profile?.characterName
    || row?.owner_id
    || '-';
}

function renderCloudCharacterCard(row, mode = 'own') {
  const summary = summarizeCharacterData(row.data || {});
  ...
  return `
    <article class="character-card">
      <div class="character-card-header">
        <div>
          <h3>${escapeHtml(row.name || summary.name)}</h3>
          <p>${escapeHtml(row.memo || 'メモなし')}</p>
        </div>
        <div class="character-meta">${activeLabel}${submittedLabel}<span>${escapeHtml(dateLabel)}</span></div>
      </div>
...
function renderCloudCharacterList() {
  const root = $('#cloudCharacterList');
  if (!root) return;
  if (!getCloudClient()) {
    root.innerHTML = '<div class="character-empty">Supabase接続を初期化できませんでした。</div>';
    return;
  }
  if (!isCloudLoggedIn()) {
    root.innerHTML = '<div class="character-empty">クラウドログインすると、保存したキャラクターを確認できます。</div>';
    return;
  }
  if (cloudLoading) {
    root.innerHTML = '<div class="character-empty">クラウド保存を読み込み中です。</div>';
    return;
  }
  if (!cloudCharacters.length) {
    root.innerHTML = '<div class="character-empty">まだクラウド保存されたキャラクターはありません。</div>';
    return;
  }
  root.innerHTML = cloudCharacters.map((row) => renderCloudCharacterCard(row, 'own')).join('');
}
...
async function refreshCloudData(options = {}) {
  ...
  try {
    ...
    console.error(error);
    cloudLastError = formatCloudError(error);
    if (!silent) toast('クラウドデータの読み込みに失敗しました');
  } finally {
    cloudLoading = false;
    updateCloudUi();
  }
...
async function loadCloudProfile(options = {}) {
  const client = getCloudClient();
  const user = getCloudUser();
  if (!client || !user) return null;
  const { data, error } = await client
  ...
  cloudProfile = data || {
    id: user.id,
    display_name: user.user_metadata?.display_name || user.email || 'クラウドユーザー',
    role: 'player'
  };
...
async function loadCloudCharacters(options = {}) {
  const client = getCloudClient();
  const user = getCloudUser();
  ...
  if (error) throw error;
  cloudCharacters = Array.isArray(data) ? data : [];
  if (options.render !== false) updateCloudUi();
  return cloudCharacters;
}

async function loadCloudAdminCharacters(options = {}) {
  const client = getCloudClient();
  if (!client || !isCloudAdmin()) return [];
  let result = await client
    .from('characters')
    .select('id, owner_id, name, memo, data, submitted, submitted_at, created_at, updated_at, profiles(display_name)')
    .eq('submitted', true)
    .order('submitted_at', { ascending: false });
  if (result.error && /relationship|schema cache|foreign key/i.test(result.error.message || '')) {
...
async function logoutCloudAccount() {
  ...
  try {
    ...
    console.error(error);
    cloudLastError = formatCloudError(error);
    toast('クラウドログアウトに失敗しました');
  } finally {
    setCloudBusy(false);
  }
}

function getCloudSaveName() {
  return ($('#librarySaveName')?.value || '').trim() || state.profile.characterName || '無名キャラクター';
}

function getCloudSaveMemo() {
  return ($('#librarySaveMemo')?.value || '').trim();
}
...
function loadCloudCharacterRow(row, options = {}) {
  ...
  state.meta = {
    ...(state.meta || {}),
    cloudId: preserveCloudId ? row.id : '',
...
function loadCloudCharacter(characterId) {
  ...
  if (!row) {
    toast('クラウド保存キャラクターが見つかりません');
    return;
  }
  loadCloudCharacterRow(row, { preserveCloudId: true });
}

function loadCloudAdminCharacter(characterId) {
  const row = cloudAdminCharacters.find((entry) => entry.id === characterId);
  if (!row) {
    toast('提出キャラクターが見つかりません');
    return;
...
async function deleteCloudCharacter(characterId) {
  ...
  if (!confirm(`クラウド保存「${row.name || '無名キャラクター'}」を削除します。よろしいですか？`)) return;
  setCloudBusy(true);
  try {
...
function handleCloudCharacterAction(event) {
